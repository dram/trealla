...
xmember(X,[H|T]) :- X = H ; xmember(X,T).

xmaplist(_,[]).
xmaplist(P,[X|Xs]) :- call(P,X), xmaplist(P,Xs).

xselect(X,[X|Tail],Tail).
xselect(X,[Head|Tail],[Head|Rest]) :- xselect(X,Tail,Rest).
...
digit(D) :- xmember(D,[1,2,3,4,5,6,7,8,9]).
