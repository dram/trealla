...
Key-value store: namespace 'kvs'
--------------------------------

The key-value store is a global, atomic, thread-safe, in-memory store that
is optionally backed to the filesystem. The maximum size for a key is the same
as the maximum functor size, which is 1000 UTF-8 characters.

	get(+Key,?Old)       - for Key unify value with Old. If Key does
							not exist then Old value is 0
	put(+Key,+New)       - for Key create with grounded term New or error
	put(+Key,?Old,+New)  - for Key unify value with Old and replace
							with grounded term New. If Key does not exist
							then Old value is 0
	inc(+Key,?Old,+V)    - for Key unify value with Old and increment
							stored value by V. If Key does not exist
							then value is 0. If old value or V is not an
							integer then fail with a type error

	erase(+Key,?Old)     - delete Key if value unifies with Old
	erase(+Key)          - delete Key

	lget(+Key,?Old)      - for Key unify value with Old. If Key does
							not exist then Old value is []
	lput(+Key,?Old,+New) - for Key unify value with Old and replace
							with grounded term New. If Key does not exist
							then Old value is []
	lpush(+Key,+V)       - push to head of list
	lpop(+Key,?V)        - pop from head of list
	lpop(+Key)           - pop from head of list
	lerase(+Key,+V)      - remove value from anywhere in list


If Key is a compound then functor/arity is used as the actual key,
otherwise the string representation is used. If a key begins with a '$'
character then it is considered a temporary and is never backed to the
filesystem. Values stored are copy_terms.

Hint: use *lput/3* to atomically push or pop a list:

	lput(K,Old,[V|Old])  - push V to head
	lput(K,[V|New],New)  - pop V from head

although in transactions it's better to use lpush/lpop if the old
or new value is not needed.

To load previously saved values:

	load/0               - load existing data

Updates are saved automatically on program exit.

Transactions within the key-value store can be done via bracketing a
series of one or more updates with *begin/end* calls:
...
and occurs as a single (all-or-nothing) update to the store and write to
the transaction log-file. A transaction locks out any other *kvs*
transactions for the duration. The *begin/0* call creates a choicepoint
and backtracking will cause it to rollback the transaction. Inside a
transaction changes to the database are not visible.
