...
static int process_check(tpl_query *q, const tpl_query *who, node *term)
{
	...
	for (node *n = NLIST_FRONT(&q->queue); n; n = term_next(n)) {
		...
		if (!unify(q, term, q->latest_context, n->val_ptr, -1)) {
			reallocate_frame(q);
			continue;
		}
...
static int bif_proc_get_keys_2(tpl_query *q)
{
	...
	if (cnt) {
		...
		ok = unify(q, term2, term2_ctx, tmp, q->c.curr_frame);
		term_heapcheck(tmp);
	}
...
static int bif_proc_get_1(tpl_query *q)
{
	...
	if (cnt) {
		term_append(l, make_const_atom("[]"));
		ok = unify(q, term1, term1_ctx, save_l, -1);
	}
	else {
		node *tmp = make_const_atom("[]");
		ok = unify(q, term1, term1_ctx, tmp, -1);
		term_heapcheck(tmp);
	}
