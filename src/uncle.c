...
static int uncle_db_add(uncle *u, const char *name, int local, const char *addr, unsigned port6, unsigned port4, int tcp, int ssl, int pri)
{
	char tmpbuf[1024];
	sprintf(tmpbuf, "%s/%s/%d/%u/%u/%d/%d/%d", name, addr, local, port6, port4, tcp, ssl, pri);
	if (g_debug) printf("DEBUG: DBADD %s\n", tmpbuf);
	lock_lock(u->strand);
...
int uncle_add(uncle *u, const char *name, const char *addr, unsigned port6, unsigned port4, int tcp, int ssl, int pri)
{
	...
	if (strlen(name) > 255) return 0;
	if (strlen(addr) > 255) return 0;
	uncle_db_add(u, name, 1, addr, port6, port4, tcp, ssl, pri);

	char tmpbuf[1024];
	sprintf(tmpbuf,
		"{\"$scope\":\"%s\",\"$unique\":%llu,\"$cmd\":\"+\",\"$name\":\"%s\",\"$port6\":%u,\"$port4\":%u,\"$tcp\":%s,\"$ssl\":%s,\"$pri\":%s}\n",
			u->scope, (unsigned long long)u->unique, name, port6, port4,
			tcp?"true":"false", ssl?"true":"false", pri?"true":"false");
	if (g_debug) printf("DEBUG: ADD %s", tmpbuf);
...
static int uncle_db_rem(uncle *u, const char *name, int local, const char *addr, int tcp)
{
	...
	u->search.ssl = -1;
	u->search.addr[0] = 0;
	if (g_debug) printf("DEBUG: DBREM %s\n", u->search.key);
...
int uncle_rem(uncle *u, const char *name, const char *addr, int tcp)
{
	...
	char tmpbuf[1024];
	sprintf(tmpbuf,
		"{\"$scope\":\"%s\",\"$unique\":%llu,\"$cmd\":\"-\",\"$name\":\"%s\",\"$tcp\":%s}\n",
			u->scope, (unsigned long long)u->unique, name, tcp?"true":"false");
	if (g_debug) printf("DEBUG: REM %s", tmpbuf);
	session_writemsg(u->s, tmpbuf);
	return 1;
}
...
static int uncle_iter2(uncle *u, const char *k, const char *v)
{
	...
	name[sizeof(name)-1] = 0;
	addr[sizeof(addr)-1] = 0;
	if (g_debug) printf("DEBUG: ??? %s, search name=%s, tcp=%d, ssl=%d\n",
	...
	sprintf(tmpbuf,
		"{\"$scope\":\"%s\",\"$unique\":%llu,\"$cmd\":\"+\",\"$name\":\"%s\",\"$port6\":%u,\"$port4\":%u,\"$tcp\":%s,\"$ssl\":%s,\"$pri\":%s}\n",
			u->scope, (unsigned long long)u->unique, name, port6, port4, tcp?"true":"false", ssl?"true":"false", pri?"true":"false");
	if (g_debug) printf("DEBUG: SND %s", tmpbuf);
...
static int uncle_handler(session *s, void *data)
{
	...
	if (g_debug) printf("*** DEBUG: handler\n");

	if (!session_readmsg(s, &buf))
		return 0;

	const char *addr = session_get_remote_addr(s, 0);
	if (g_debug) printf("DEBUG: RCV %s: %s", addr, buf);
	char scope[256];
...
uncle *uncle_create2(handler *h, const char *binding, unsigned port, const char *scope, const char *maddr6, const char *maddr4)
{
	...
	if (g_debug) printf("DEBUG: CREATE %s", tmpbuf);
