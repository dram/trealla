#include <ctype.h>
#include <errno.h>
#include <float.h>
#include <limits.h>
#include <math.h>
#include <stddef.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>
#include <time.h>

#ifdef _WIN32
#include <io.h>
#include <winsock2.h>
#define snprintf _snprintf
#define fseeko _fseeki64
#define msleep Sleep
#else
#include <arpa/inet.h>
#include <netinet/in.h>
#include <unistd.h>
#define msleep(ms)                                                                                                             \
	{                                                                                                                          \
		struct timespec tv;                                                                                                    \
		tv.tv_sec = (ms) / 1000;                                                                                               \
		tv.tv_nsec = ((ms) % 1000) * 1000 * 1000;                                                                              \
		nanosleep(&tv, &tv);                                                                                                   \
	}
#endif

#ifndef USE_SSL
#define USE_SSL 0
#endif

#if USE_SSL
#include "openssl/sha.h"
#endif

#include "trealla.h"

#include "base64.h"
#include "bifs.h"
#include "jela.h"

#define MAX_CHUNK_SIZE (1024LL * 1024 * 1024) // 1GB
#define MAX_FORM_SIZE (1024LL * 1024 * 64)    // 64MB

static char *sanitize(const char *path, char *path2)
{
	const char *src = path;
	char *dst = path2;

	while (isspace(*src))
		src++;

	while (*src) {
		if ((src[0] == '\'') || (src[0] == '\"') || (src[0] == '~'))
			;
		else if ((src[0] != '.') || (src[1] != '.'))
			*dst++ = *src;

		src++;
	}

	while ((dst != path2) && isspace(*(dst - 1)))
		dst--;

	*dst = '\0';
	return path2;
}

static char *http_cleanup(const char *path, char *path2)
{
	const char *src = path;
	char *dst = path2;

	while (isspace(*src))
		src++;

	while (*src) {
		if ((src[0] == '.') && (src[1] == '.'))
			src += 2;
		else
			*dst++ = *src++;
	}

	while ((dst != path2) && isspace(*(dst - 1)))
		dst--;

	*dst = '\0';
	return path2;
}

static int bif_http_query_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char tmpbuf[KEY_SIZE];
	snprintf(tmpbuf, KEY_SIZE - 20, "QUERY:%s", VAL_S(term2));
	const char *src = session_get_stash(s, tmpbuf);
	put_atom(q, term3, term3_ctx, strdup(src));
	return 1;
}

static int bif_http_form_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char tmpbuf[KEY_SIZE];
	snprintf(tmpbuf, KEY_SIZE - 20, "FORM:%s", VAL_S(term2));
	const char *src = session_get_stash(s, tmpbuf);
	put_atom(q, term3, term3_ctx, strdup(src));
	return 1;
}

static int bif_http_cookie_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char tmpbuf[KEY_SIZE];
	snprintf(tmpbuf, KEY_SIZE - 20, "COOKIE:%s", VAL_S(term2));
	const char *src = session_get_stash(s, tmpbuf);
	put_atom(q, term3, term3_ctx, strdup(src));
	return 1;
}

static int bif_http_query_list_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char tmpbuf[KEY_SIZE];
	snprintf(tmpbuf, KEY_SIZE - 20, "QUERY:%s", VAL_S(term2));
	node *save_l = NULL, *l = NULL;
	const char *v = session_get_key(s, tmpbuf);

	while (v) {
		if (!save_l)
			save_l = l = make_list();

		term_append(l, make_atom(strdup(v)));
		v = session_get_key(s, tmpbuf);

		if (!v)
			break;

		l = term_append(l, make_list());
	}

	if (l)
		term_append(l, make_const_atom("[]"));
	else
		save_l = make_const_atom("[]");

	put_env(q, term3, term3_ctx, save_l, -1);
	term_heapcheck(save_l);
	return 1;
}

static int bif_http_form_list_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char tmpbuf[KEY_SIZE];
	snprintf(tmpbuf, KEY_SIZE - 20, "FORM:%s", VAL_S(term2));
	node *save_l = NULL, *l = NULL;
	const char *v = session_get_key(s, tmpbuf);

	while (v) {
		if (!save_l)
			save_l = l = make_list();

		term_append(l, make_atom(strdup(v)));
		v = session_get_key(s, tmpbuf);

		if (!v)
			break;

		l = term_append(l, make_list());
	}

	if (l)
		term_append(l, make_const_atom("[]"));
	else
		save_l = make_const_atom("[]");

	put_env(q, term3, term3_ctx, save_l, -1);
	term_heapcheck(save_l);
	return 1;
}

static int bif_http_cookie_list_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char tmpbuf[KEY_SIZE];
	snprintf(tmpbuf, KEY_SIZE - 20, "COOKIE:%s", VAL_S(term2));
	node *save_l = NULL, *l = NULL;
	const char *v = session_get_key(s, tmpbuf);

	while (v) {
		if (!save_l)
			save_l = l = make_list();

		term_append(l, make_atom(strdup(v)));
		v = session_get_key(s, tmpbuf);

		if (!v)
			break;

		l = term_append(l, make_list());
	}

	if (l)
		term_append(l, make_const_atom("[]"));
	else
		save_l = make_const_atom("[]");

	put_env(q, term3, term3_ctx, save_l, -1);
	term_heapcheck(save_l);
	return 1;
}

static int bif_http_www_form_1(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	stream *sp = term1->val_str;
	session *s = sp->sptr;

	if (session_on_disconnect(s)) {
		q->is_yielded = 1;
		return 0;
	}

	long long nbytes = session_get_stash_int(s, "Content-Length");

	if (nbytes > MAX_FORM_SIZE) {
		session_close(s);
		return 0;
	}

	long save_len = (long)nbytes;
	char *bufptr = (char *)malloc(nbytes + 1);
	char *dst = bufptr;

	while (nbytes > 0) {
		int len = session_read(s, dst, nbytes);
		dst += len;
		*dst = '\0';
		nbytes -= len;

		if (!len)
			msleep(1);
	}

	const char *src = bufptr;
	char *name = (char *)malloc(save_len + 1);
	char *value = (char *)malloc(save_len + 1);
	char *tmpname = (char *)malloc(save_len + 1);
	char *tmpvalue = (char *)malloc(save_len + 1);
	dst = name;

	while (*src) {
		if (*src == '=') {
			*dst = '\0';
			dst = value;
			src++;
		}
		else if (*src == '&') {
			*dst = '\0';
			char tmpbuf[KEY_SIZE], tmpbuf1[KEY_SIZE], tmpbuf2[KEY_SIZE];
			snprintf(tmpbuf, KEY_SIZE - 20, "FORM:%s", url_decode(name, tmpbuf1));
			session_set_stash(s, tmpbuf, url_decode(value, tmpbuf2));
			dst = name;
			src++;
		}
		else
			*dst++ = *src++;
	}

	*dst = '\0';

	if (name[0]) {
		char tmpbuf[KEY_SIZE], tmpbuf1[KEY_SIZE], tmpbuf2[KEY_SIZE];
		snprintf(tmpbuf, KEY_SIZE - 20, "FORM:%s", url_decode(name, tmpbuf1));
		session_set_stash(s, tmpbuf, url_decode(value, tmpbuf2));
	}

	free(tmpvalue);
	free(tmpname);
	free(value);
	free(name);
	free(bufptr);
	return 1;
}

static void parse_cookies(session *s, const char *src)
{
	size_t save_len = strlen(src);
	char *name = (char *)malloc(save_len + 1);
	char *value = (char *)malloc(save_len + 1);
	char *dst = name;
	name[0] = value[0] = '\0';
	int quoted = 0, named = 0;

	while (*src) {
		char ch = *src++;

		if ((ch == '=') && !named) {
			*dst = '\0';
			dst = value;
			*dst = '\0';
			named = 1;

			if (*src == '"') {
				quoted = 1;
				src++;
				continue;
			}
		}
		else if (quoted && (ch == '"')) {
			quoted = 0;
			continue;
		}
		else if (ch == ';') {
			named = 0;
			*dst = '\0';
			char tmpbuf[KEY_SIZE];
			snprintf(tmpbuf, KEY_SIZE - 20, "COOKIE:%s", name);
			session_app_stash(s, tmpbuf, value);
			//printf("*** '%s' = '%s'\n", name, value);
			dst = name;
			*dst = '\0';
			value[0] = '\0';

			while (isspace(*src))
				src++;

			continue;
		}

		*dst++ = ch;
	}

	*dst = '\0';

	if (name[0]) {
		char tmpbuf[KEY_SIZE];
		snprintf(tmpbuf, KEY_SIZE - 20, "COOKIE:%s", name);
		//printf("*** '%s' = '%s'\n", name, value);
		session_app_stash(s, tmpbuf, value);
	}

	free(name);
	free(value);
}

static void parse_header(session *s, char *bufptr, int len)
{
	const char *src = strchr(bufptr, ':');

	if (!src) {
		free(bufptr);
		return;
	}

	src = bufptr;
	char *name = (char *)malloc(len + 1);
	char *dst = name;

	while (isspace(*src))
		src++;

	while (*src && !isspace(*src) && (*src != ':')) {
		*dst++ = tolower(*src);
		src++;
	}

	*dst = '\0';

	while (isspace(*src))
		src++;

	src++; // the ':'

	while (isspace(*src))
		src++;

	char *value = (char *)malloc(len + 1);
	dst = value;

	while (*src && (*src != '\r') && (*src != '\n'))
		*dst++ = *src++;

	while ((dst != value) && isspace(*(dst - 1)))
		dst--;

	*dst = '\0';
	free(bufptr);
	session_set_stash(s, name, value);

	if (!strcmp(name, "set-cookie") ||!strcmp(name, "cookie"))
		parse_cookies(s, value);

	free(name);
	free(value);
}

static int bif_http_parse_4(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_term(term1);
	node *term2 = get_var(term2);
	node *term3 = get_var(term3);
	node *term4 = get_var(term4);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char *bufptr = NULL;

	if (!is_socket(term1)) {
		q->halt_code = 1;
		q->halt = ABORT_HALT;
		return 0;
	}

	if (session_on_disconnect(s)) {
		q->is_yielded = 1;
		return 0;
	}

	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		// printf("***> %s", bufptr);

		if (!session_get_udata_flag(s, CMD)) {
			session_clr_stash(s);
			session_set_udata_flag(s, CMD);

			char cmd[256], ver[20];
			char *path = (char *)malloc(len + 1);
			char *path2 = (char *)malloc(len + 1);
			char *full_path = (char *)malloc(len + 1);
			cmd[0] = path[0] = full_path[0] = path2[0] = ver[0] = '\0';
			sscanf(bufptr, "%255s %s HTTP/%19s", cmd, full_path, ver);
			free(bufptr);
			cmd[sizeof(cmd) - 1] = ver[sizeof(ver) - 1] = full_path[len] = '\0';
			sscanf(full_path, "%[^? \r\n]", path);
			url_decode(path, path2);
			sanitize(path2, path);
			free(path2);
			const char *ptr = strchr(full_path, '?');

			if (session_is_tls(s))
				session_set_stash(s, "SERVER_PROTOCOL", "https");
			else
				session_set_stash(s, "SERVER_PROTOCOL", "http");

			session_set_stash(s, "REQUEST_METHOD", cmd);
			session_set_stash(s, "HTTP", ver);
			session_set_stash(s, "PATH_INFO", path);
			session_set_stash(s, "PATH_TRANSLATED", path);
			session_set_stash(s, "QUERY_STRING", ptr ? ptr : "");

			if (session_is_ipv6(s)) {
				char tmpbuf[256];
				snprintf(tmpbuf, sizeof(tmpbuf), "[%s]", session_get_remote_addr(s, 0));
				session_set_stash(s, "REMOTE_ADDR", tmpbuf);
				session_set_stash(s, "REMOTE_HOST", tmpbuf);
				snprintf(tmpbuf, sizeof(tmpbuf), "[%s]", session_get_local_addr(s, 0));
				session_set_stash(s, "LOCAL_ADDR", tmpbuf);
				session_set_stash(s, "LOCAL_HOST", tmpbuf);
			}
			else {
				session_set_stash(s, "REMOTE_ADDR", session_get_remote_addr(s, 0));
				session_set_stash(s, "REMOTE_HOST", session_get_remote_addr(s, 0));
				session_set_stash(s, "LOCAL_ADDR", session_get_local_addr(s, 0));
				session_set_stash(s, "LOCAL_HOST", session_get_local_addr(s, 0));
			}

			session_set_stash_int(s, "REMOTE_PORT", session_get_remote_port(s));
			session_set_stash_int(s, "LOCAL_PORT", session_get_local_port(s));

			if (ptr) {
				char *name = (char *)malloc(len + 64);
				char *value = (char *)malloc(len + 1);
				const char *src = ptr + 1;
				char *dst = name;

				while (*src) {
					if (*src == '=') {
						*dst = '\0';
						dst = value;
						src++;
					}
					else if (*src == '&') {
						*dst = '\0';
						char tmpbuf[KEY_SIZE], tmpbuf1[KEY_SIZE], tmpbuf2[KEY_SIZE];
						snprintf(tmpbuf, KEY_SIZE - 20, "QUERY:%s", url_decode(name, tmpbuf1));
						session_app_stash(s, tmpbuf, url_decode(value, tmpbuf2));
						dst = name;
						src++;
					}
					else
						*dst++ = *src++;
				}

				*dst = '\0';

				if (name[0]) {
					char tmpbuf[KEY_SIZE], tmpbuf1[KEY_SIZE], tmpbuf2[KEY_SIZE];
					snprintf(tmpbuf, KEY_SIZE - 20, "QUERY:%s", url_decode(name, tmpbuf1));
					session_app_stash(s, tmpbuf, url_decode(value, tmpbuf2));
				}

				free(name);
				free(value);
			}

			free(path);
			free(full_path);
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);
			char tmpbuf[1024], tmpbuf2[1024], tmpbuf3[256];
			tmpbuf2[0] = tmpbuf3[0] = '\0';
			const char *src = session_get_stash(s, "Host");

			if (*src)
				sscanf(src, "%1023[^: \r\n]:%255s", tmpbuf2, tmpbuf3);

			tmpbuf2[sizeof(tmpbuf2) - 1] = tmpbuf3[sizeof(tmpbuf3) - 1] = '\0';

			if (tmpbuf2[0])
				session_set_stash(s, "SERVER_NAME", http_cleanup(tmpbuf2, tmpbuf));
			else
				session_set_stash(s, "SERVER_NAME", session_get_stash(s, "LOCAL_ADDRESS"));

			if (tmpbuf3[0])
				session_set_stash(s, "SERVER_PORT", http_cleanup(tmpbuf3, tmpbuf));
			else
				session_set_stash(s, "SERVER_PORT", session_get_stash(s, "LOCAL_PORT"));

			if (strlen(session_get_stash(s, "Authorization"))) {
				const char *hdr = session_get_stash(s, "Authorization");
				char auth[256], tmpbuf[256];
				auth[0] = tmpbuf[0] = '\0';
				sscanf(hdr, "%255s %255s", auth, tmpbuf);
				auth[sizeof(auth) - 1] = tmpbuf[sizeof(tmpbuf) - 1] = '\0';

				if (!strcmp(auth, "Basic")) {
					size_t nbytes = 64;
					char *dstbuf = (char *)malloc(nbytes);
					b64_decode(tmpbuf, strlen(tmpbuf), (char **)&dstbuf);
					char userid[256], passwd[256];
					userid[0] = passwd[0] = '\0';
					sscanf(dstbuf, "%255[^:]:%255s", userid, passwd);
					userid[sizeof(userid) - 1] = passwd[sizeof(passwd) - 1] = '\0';
					session_set_stash(s, "USER", userid);
					session_set_stash(s, "PASS", passwd);
					free(dstbuf);
				}
			}

			if (strlen(session_get_stash(s, "WWW-Authenticate"))) {
				const char *hdr = session_get_stash(s, "WWW-Authenticate");
				char auth[256], tmpbuf[256];
				auth[0] = tmpbuf[0] = '\0';
				sscanf(hdr, "%255s %255s", auth, tmpbuf);
				auth[sizeof(auth) - 1] = tmpbuf[sizeof(tmpbuf) - 1] = '\0';

				if (!strcmp(auth, "Basic") && !strncmp(tmpbuf, "Realm", 5)) {
					const char *src = tmpbuf;
					char *dst = auth;

					while (isalpha(*src) || (*src == '='))
						src++;

					if (*src == '"')
						src++;

					while (*src && (*src != '"') && ((dst - auth) < (sizeof(auth) - 1)))
						*dst++ = *src++;

					*dst = '\0';
					session_set_stash(s, "REALM", auth);
				}
			}

			if (strlen(session_get_stash(s, "WWW-Authenticate")))
				session_set_stash(s, "WWW_AUTHENTICATE",
				                  http_cleanup(session_get_stash(s, "WWW-Authenticate"), tmpbuf));
			if (strlen(session_get_stash(s, "Authorization")))
				session_set_stash(s, "AUTHORIZATION",
				                  http_cleanup(session_get_stash(s, "Authorization"), tmpbuf));
			if (strlen(session_get_stash(s, "Referer")))
				session_set_stash(s, "REFERER",
				                  http_cleanup(session_get_stash(s, "Referer"), tmpbuf));
			if (strlen(session_get_stash(s, "User-Agent")))
				session_set_stash(s, "USER_AGENT",
				                  http_cleanup(session_get_stash(s, "User-Agent"), tmpbuf));
			if (strlen(session_get_stash(s, "Upgrade")))
				session_set_stash(s, "UPGRADE",
				                  http_cleanup(session_get_stash(s, "Upgrade"), tmpbuf));
			if (strlen(session_get_stash(s, "Origin")))
				session_set_stash(s, "ORIGIN",
				                  http_cleanup(session_get_stash(s, "Origin"), tmpbuf));

			if (strlen(session_get_stash(s, "Connection")))
				;
			else if (atof(session_get_stash(s, "HTTP")) < 1.1)
				session_set_stash(s, "Connection", "close");
			else
				session_set_stash(s, "Connection", "keep-alive");

			if (strlen(session_get_stash(s, "Content-Length")))
				session_set_stash(s, "CONTENT_LENGTH",
				                  session_get_stash(s, "Content-Length"));
			if (strlen(session_get_stash(s, "Content-Type")))
				session_set_stash(s, "CONTENT_TYPE", session_get_stash(s, "Content-Type"));

			put_float(q, term2, term2_ctx, (double)atof(session_get_stash(s, "HTTP")));
			put_atom(q, term3, term3_ctx, strdup(session_get_stash(s, "REQUEST_METHOD")));
			put_atom(q, term4, term4_ctx, strdup(session_get_stash(s, "PATH_INFO")));
			return 1;
		}

		parse_header(s, bufptr, len);
	}

	q->is_yielded = 1;
	return 0;
}

static node *make_options_list(session *s)
{
	node *l = make_list();
	node *save_l = l, *tmp, *n;
	const char *src;

	n = make_compound();
	term_append(n, make_const_atom("length"));
	term_append(n, make_int(session_get_stash_int(s, "Content-Length")));
	term_append(l, n);

	tmp = make_list();
	n = make_compound();
	term_append(n, make_const_atom("version"));
	term_append(n, make_float(session_get_stash_float(s, "HTTP")));
	term_append(tmp, n);
	term_append(l, tmp);
	l = tmp;

	tmp = make_list();
	n = make_compound();
	term_append(n, make_const_atom("method"));
	src = session_get_stash(s, "REQUEST_METHOD");
	term_append(n, make_atom(strdup(src)));
	term_append(tmp, n);
	term_append(l, tmp);
	l = tmp;

	src = session_get_stash(s, "Connection");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("persist"));
		term_append(n, make_const_atom(!strcmp(src, "close") ? "false" : "true"));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Content-Type");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("type"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "If-Modified-Since");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("modified"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Last-Modified");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("modified"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Referer");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("referer"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Cookie");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("cookie"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Set-Cookie");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("cookie"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Transfer-Encoding");

	if ((src && *src) && strstr(src, "chunked")) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("chunked"));
		term_append(n, make_const_atom("true"));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	src = session_get_stash(s, "Server");

	if (src && *src) {
		tmp = make_list();
		n = make_compound();
		term_append(n, make_const_atom("agent"));
		term_append(n, make_atom(strdup(src)));
		term_append(tmp, n);
		term_append(l, tmp);
		l = tmp;
	}

	term_append(l, make_const_atom("[]"));
	return save_l;
}

static int bif_http_parse_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_term(term1);
	node *term2 = get_var(term2);
	node *term3 = get_next_arg(q, &args);
	unsigned term3_ctx = q->c.curr_frame;
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	int debug = session_get_stash_int(s, "DEBUG");

	if (!is_socket(term1)) {
		q->halt_code = 1;
		q->halt = ABORT_HALT;
		return 0;
	}

	if (session_on_disconnect(s)) {
		q->is_yielded = 1;
		return 0;
	}

	char *bufptr = NULL;
	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		if (debug)
			printf("%s", bufptr);

		if (!session_get_udata_flag(s, CMD)) {
			session_set_udata_flag(s, CMD);
			int status = 0;
			char ver[20];
			ver[0] = '\0';
			sscanf(bufptr, "HTTP/%19s %d", ver, &status);
			free(bufptr);
			ver[sizeof(ver) - 1] = '\0';
			put_int(q, term2, term2_ctx, status);
			char tmpbuf[20];
			snprintf(tmpbuf, sizeof(tmpbuf), "%d", status);
			session_set_stash(s, "X_STATUS", tmpbuf);
			session_set_stash(s, "HTTP", ver);
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);

			if (strlen(session_get_stash(s, "Content-Length")))
				session_set_stash(s, "CONTENT_LENGTH", session_get_stash(s, "Content-Length"));

			if (strlen(session_get_stash(s, "Content-Type")))
				session_set_stash(s, "CONTENT_TYPE", session_get_stash(s, "Content-Type"));

			if (term3 && is_var(term3)) {
				node *l = make_options_list(s);
				put_env(q, term3, term3_ctx, l, -1);
				term_heapcheck(l);
			}

			return 1;
		}

		parse_header(s, bufptr, len);
	}

	return 0;
}

#define OPTION_NAME_LEN (1024 * 4)

typedef struct {
	char type[OPTION_NAME_LEN], agent[OPTION_NAME_LEN], method[OPTION_NAME_LEN];
	char modified[OPTION_NAME_LEN], cookie[OPTION_NAME_LEN], referer[OPTION_NAME_LEN];
	double version;
	long length;
	int persist, debug, chunked, hdrlen;
	char hdrs[1024 * 8];
}
 options;

static options *init_options(void)
{
	options *opt = (options *)calloc(1, sizeof(options));
	opt->version = 1.1;
	opt->persist = 1;
	opt->length = -1;
	strcpy(opt->agent, "Trealla");
	return opt;
}

static void parse_option(tpl_query *q, options *opt, node *n)
{
	if (!is_compound(n))
		return;

	n = term_first(n);
	node *v = term_next(n);
	n = subst(q, n, q->latest_context);
	v = subst(q, v, q->latest_context);
	const char *f = VAL_S(n);

	if (!strcmp(f, "version")) {
		if (is_float(v) && (v->val_f != 1.1)) {
			opt->version = 1.0;
			opt->persist = 0;
		}

		return;
	}

	if (!strcmp(f, "length")) {
		if (is_integer(v))
			opt->length = VAL_INT(v);

		return;
	}

	if (!strcmp(f, "persist")) {
		if (is_atom(v) && !strcmp(VAL_S(v), "true"))
			opt->persist = 1;
		else if (is_atom(v) && !strcmp(VAL_S(v), "false"))
			opt->persist = 0;
		else if (is_integer(v) && (VAL_INT(v) == 1))
			opt->persist = 1;
		else if (is_integer(v) && (VAL_INT(v) == 0))
			opt->persist = 0;

		return;
	}

	if (!strcmp(f, "chunked")) {
		if (is_atom(v) && !strcmp(VAL_S(v), "true"))
			opt->chunked = 1;
		else if (is_integer(v) && (VAL_INT(v) == 1))
			opt->chunked = 1;

		return;
	}

	if (!strcmp(f, "debug")) {
		if (is_atom(v) && !strcmp(VAL_S(v), "true"))
			opt->debug = 1;
		else if (is_integer(v) && (VAL_INT(v) == 1))
			opt->debug = 1;

		return;
	}

	if (!strcmp(f, "type")) {
		if (is_atom(v)) {
			strncpy(opt->type, VAL_S(v), OPTION_NAME_LEN);
			opt->type[OPTION_NAME_LEN-1] = '\0';
		}

		return;
	}

	if (!strcmp(f, "modified")) {
		if (is_atom(v)) {
			strncpy(opt->modified, VAL_S(v), OPTION_NAME_LEN);
			opt->modified[OPTION_NAME_LEN-1] = '\0';
		}

		return;
	}

	if (!strcmp(f, "referer")) {
		if (is_atom(v)) {
			strncpy(opt->referer, VAL_S(v), OPTION_NAME_LEN);
			opt->referer[OPTION_NAME_LEN-1] = '\0';
		}

		return;
	}

	if (!strcmp(f, "cookie")) {
		if (is_atom(v)) {
			strncpy(opt->cookie, VAL_S(v), OPTION_NAME_LEN);
			opt->cookie[OPTION_NAME_LEN-1] = '\0';
		}

		return;
	}

	if (!strcmp(f, "agent")) {
		if (is_atom(v)) {
			strncpy(opt->agent, VAL_S(v), OPTION_NAME_LEN);
			opt->agent[OPTION_NAME_LEN-1] = '\0';
		}

		return;
	}

	if (!strcmp(f, "method")) {
		if (is_atom(v)) {
			strncpy(opt->method, VAL_S(v), OPTION_NAME_LEN);
			opt->method[OPTION_NAME_LEN-1] = '\0';
		}

		return;
	}

	if (!strcmp(f, "header")) {
		if (is_structure(v) && !strcmp(term_functor(v), ":")) {
			node *name = term_firstarg(v);
			node *value = term_next(name);
			opt->hdrlen += term_sprint(opt->hdrs+opt->hdrlen, sizeof(opt->hdrs)-opt->hdrlen, q->pl, q, name, 0);

			if ((sizeof(opt->hdrs)-opt->hdrlen) > 10) {
				opt->hdrs[opt->hdrlen++] = ':';
				opt->hdrs[opt->hdrlen++] = ' ';
			}

			opt->hdrlen += term_sprint(opt->hdrs+opt->hdrlen, sizeof(opt->hdrs)-opt->hdrlen, q->pl, q, value, 0);

			if ((sizeof(opt->hdrs)-opt->hdrlen) > 10) {
				opt->hdrs[opt->hdrlen++] = '\r';
				opt->hdrs[opt->hdrlen++] = '\n';
				opt->hdrs[opt->hdrlen] = '\0';
			}
			else
				opt->hdrlen = 0;
		}

		return;
	}
}

static options *parse_options_list(tpl_query *q, node *args, node *l)
{
	options *opt = init_options();

	if (!l)
		return opt;

	while (is_list(l)) {
		node *head = term_firstarg(l);
		unsigned this_context = q->latest_context;
		node *n = subst(q, head, this_context);
		parse_option(q, opt, n);
		node *tail = term_next(head);
		l = subst(q, tail, this_context);
	}

	return opt;
}

static char *parse_hdrs_list(tpl_query *q, node *args, node *l)
{
	size_t dstlen = 1024 * 8;
	char *dstbuf = (char*)malloc(dstlen);
	char *dst = dstbuf;

	while (is_list(l)) {
		node *head = term_firstarg(l);
		unsigned this_context = q->latest_context;
		node *n = subst(q, head, this_context);

		if (is_compound(n) && !strcmp(term_functor(n), "{}")) {
			node *n2 = term_firstarg(n);

			if (is_compound(n2) && !strcmp(term_functor(n2), ":")) {
				node *n = term_firstarg(n2);
				dst += term_sprint2(&dstbuf, &dstlen, &dst, q->pl, q, n, 0);
				*dst++ = ':';
				*dst++ = ' ';
				n = term_next(n);
				dst += term_sprint2(&dstbuf, &dstlen, &dst, q->pl, q, n, 0);
			}
		}
		else if (is_compound(n) && !strcmp(term_functor(n), ":")) {
			node *n2 = term_firstarg(n);
			dst += term_sprint2(&dstbuf, &dstlen, &dst, q->pl, q, n2, 0);
			*dst++ = ':';
			*dst++ = ' ';
			n2 = term_next(n2);
			dst += term_sprint2(&dstbuf, &dstlen, &dst, q->pl, q, n2, 0);
		}
		else
			dst += term_sprint2(&dstbuf, &dstlen, &dst, q->pl, q, n, 0);

		*dst++ = '\r';
		*dst++ = '\n';
		node *tail = term_next(head);
		l = subst(q, tail, this_context);
	}

	*dst = '\0';
	return dstbuf;
}

static int http_request(const char *cmd, session *s, const char *path, options *opt, char *hdrs)
{
	const char *host = session_get_stash(s, "HOST");
	const char *user = session_get_stash(s, "USER");
	const char *pass = session_get_stash(s, "PASS");

	if (opt->method[0])
		cmd = opt->method;

	char dstbuf[1024 * 8];
	char *dst = dstbuf;
	dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "%s %s HTTP/%g\r\n", cmd, path, opt->version);
	dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Host: %s\r\n", host);
	dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "User-Agent: %s\r\n", opt->agent);

	if (user[0]) {
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Authorization: Basic ");
		char tmpbuf[1024];
		snprintf(tmpbuf, sizeof(tmpbuf), "%s:%s", user, pass);
		dst += b64_encode(tmpbuf, strlen(tmpbuf), &dst, 0, 0);
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "\r\n");
	}

	if (!strcmp(cmd, "GET") || !strcmp(cmd, "DELETE"))
		opt->length = 0;

	if (opt->length >= 0)
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Content-Length: %ld\r\n", opt->length);
	else if (opt->version > 1.0)
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Transfer-Encoding: chunked\r\n");

	if (opt->persist)
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Connection: %s\r\n", "keep-alive");
	else
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Connection: %s\r\n", "close");

	if ((opt->version > 1.0) && opt->chunked)
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Accept-Encoding: chunked\r\n");

	if (opt->type[0])
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Content-Type: %s\r\n", opt->type);

	if (opt->referer[0])
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Referer: %s\r\n", opt->referer);

	if (opt->modified[0])
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "If-Modified-Since: %s\r\n", opt->modified);

	if (opt->cookie[0])
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "Cookie: %s\r\n", opt->cookie);

	if (opt->hdrlen)
		dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "%s", opt->hdrs);

	if (hdrs) {
		if (strlen(hdrs) < (1024*8))
			dst += snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "%s", hdrs);

		free(hdrs);
	}

	snprintf(dst, sizeof(dstbuf)-(dst-dstbuf), "\r\n");
	session_writemsg(s, dstbuf);

	if (opt->debug)
		printf("%s", dstbuf);

	session_set_stash_int(s, "DEBUG", opt->debug);
	session_set_stash(s, "REQUEST_METHOD", cmd);
	session_set_stash(s, "User-Agent", opt->agent);
	free(opt);
	return 1;
}

static int bif_http_head_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_next_arg(q, &args);
	node *term4 = NULL;
	options *opt = NULL;
	char *hdrs = NULL;

	if (term3 && is_atom(term3) && strcmp(VAL_S(term3), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3 && !is_atom(term3) && !is_list(term3)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3)
		term4 = get_next_arg(q, &args);

	if (term4 && is_atom(term4) && strcmp(VAL_S(term4), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term4 && !is_atom(term4) && !is_list(term4)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	opt = parse_options_list(q, args, term3);

	if (term4 && is_list(term4))
		hdrs = parse_hdrs_list(q, args, term4);

	return http_request("HEAD", term1->val_str->sptr, VAL_S(term2), opt, hdrs);
}

static int bif_http_get_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_next_arg(q, &args);
	node *term4 = NULL;
	options *opt = NULL;
	char *hdrs = NULL;

	if (term3 && is_atom(term3) && strcmp(VAL_S(term3), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3 && !is_atom(term3) && !is_list(term3)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3)
		term4 = get_next_arg(q, &args);

	if (term4 && is_atom(term4) && strcmp(VAL_S(term4), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term4 && !is_atom(term4) && !is_list(term4)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	opt = parse_options_list(q, args, term3);

	if (term4 && is_list(term4))
		hdrs = parse_hdrs_list(q, args, term4);

	return http_request("GET", term1->val_str->sptr, VAL_S(term2), opt, hdrs);
}

static int bif_http_post_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_next_arg(q, &args);
	node *term4 = NULL;
	options *opt = NULL;
	char *hdrs = NULL;

	if (term3 && is_atom(term3) && strcmp(VAL_S(term3), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3 && !is_atom(term3) && !is_list(term3)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3)
		term4 = get_next_arg(q, &args);

	if (term4 && is_atom(term4) && strcmp(VAL_S(term4), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term4 && !is_atom(term4) && !is_list(term4)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	opt = parse_options_list(q, args, term3);

	if (term4 && is_list(term4))
		hdrs = parse_hdrs_list(q, args, term4);

	return http_request("POST", term1->val_str->sptr, VAL_S(term2), opt, hdrs);
}

static int bif_http_put_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_next_arg(q, &args);
	node *term4 = NULL;
	options *opt = NULL;
	char *hdrs = NULL;

	if (term3 && is_atom(term3) && strcmp(VAL_S(term3), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3 && !is_atom(term3) && !is_list(term3)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3)
		term4 = get_next_arg(q, &args);

	if (term4 && is_atom(term4) && strcmp(VAL_S(term4), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term4 && !is_atom(term4) && !is_list(term4)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	opt = parse_options_list(q, args, term3);

	if (term4 && is_list(term4))
		hdrs = parse_hdrs_list(q, args, term4);

	return http_request("PUT", term1->val_str->sptr, VAL_S(term2), opt, hdrs);
}

static int bif_http_delete_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_next_arg(q, &args);
	node *term4 = NULL;
	options *opt = NULL;
	char *hdrs = NULL;

	if (term3 && is_atom(term3) && strcmp(VAL_S(term3), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3 && !is_atom(term3) && !is_list(term3)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term3)
		term4 = get_next_arg(q, &args);

	if (term4 && is_atom(term4) && strcmp(VAL_S(term4), "[]")) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	if (term4 && !is_atom(term4) && !is_list(term4)) {
		QABORT(ABORT_INVALIDARGNOTLIST);
		return 0;
	}

	opt = parse_options_list(q, args, term3);

	if (term4 && is_list(term4))
		hdrs = parse_hdrs_list(q, args, term4);

	return http_request("DELETE", term1->val_str->sptr, VAL_S(term2), opt, hdrs);
}

int http_get10(session *s, const char *path, int keep, int *status, int head)
{
	options *opt = init_options();
	opt->version = 1.0;
	opt->length = 0;
	http_request(head ? "HEAD" : "GET", s, path, opt, "");
	char *bufptr = NULL;
	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		if (!session_get_udata_flag(s, CMD)) {
			session_set_udata_flag(s, CMD);
			*status = 0;
			char ver[20];
			ver[0] = '\0';
			sscanf(bufptr, "HTTP/%19s %d", ver, status);
			free(bufptr);
			ver[sizeof(ver) - 1] = '\0';
			char tmpbuf[20];
			snprintf(tmpbuf, sizeof(tmpbuf), "%d", *status);
			session_set_stash(s, "X_STATUS", tmpbuf);
			session_set_stash(s, "HTTP", ver);
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);

			if (strlen(session_get_stash(s, "Content-Length")))
				session_set_stash(s, "CONTENT_LENGTH", session_get_stash(s, "Content-Length"));

			if (strlen(session_get_stash(s, "Content-Type")))
				session_set_stash(s, "CONTENT_TYPE", session_get_stash(s, "Content-Type"));
			return 1;
		}

		parse_header(s, bufptr, len);
	}

	return 1;
}

int http_get11(session *s, const char *path, int keep, int *status, int head)
{
	options *opt = init_options();
	opt->version = 1.1;
	opt->length = 0;
	http_request(head ? "HEAD" : "GET", s, path, opt, "");
	char *bufptr = NULL;
	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		if (!session_get_udata_flag(s, CMD)) {
			session_set_udata_flag(s, CMD);
			*status = 0;
			char ver[20];
			ver[0] = '\0';
			sscanf(bufptr, "HTTP/%19s %d", ver, status);
			free(bufptr);
			ver[sizeof(ver) - 1] = '\0';
			char tmpbuf[20];
			snprintf(tmpbuf, sizeof(tmpbuf), "%d", *status);
			session_set_stash(s, "X_STATUS", tmpbuf);
			session_set_stash(s, "HTTP", ver);
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);

			if (strlen(session_get_stash(s, "Content-Length")))
				session_set_stash(s, "CONTENT_LENGTH", session_get_stash(s, "Content-Length"));

			if (strlen(session_get_stash(s, "Content-Type")))
				session_set_stash(s, "CONTENT_TYPE", session_get_stash(s, "Content-Type"));
			return 1;
		}

		parse_header(s, bufptr, len);
	}

	return 1;
}

static int bif_http_get_chunk_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_var(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char *line = NULL;

	// Read length line

	if (!session_readmsg(s, &line)) {
		q->is_yielded = 1;
		return 0;
	}

	// printf("+$$$ %s", line);
	unsigned long long len = 0;
	sscanf(line, "%LX", &len);
	free(line);

	if (len > MAX_CHUNK_SIZE) {
		session_close(s);
		return 0;
	}

	put_int(q, term3, term3_ctx, len);
	int save_len = len;

	// Read for specified length

	char *bufptr = (char *)malloc(len + 1);
	char *dst = bufptr;

	while (len > 0) {
		size_t rlen = 0;

		if (!(rlen = session_read(s, dst, len))) {
			if (session_on_disconnect(s)) {
				free(bufptr);
				return 0;
			}
		}

		// printf("@@@ len=%d, rlen=%d\n", len, (int)rlen);
		dst += rlen;
		len -= rlen;

		if (len && !rlen)
			msleep(1);
	}

	*dst = '\0';
	node *tmp = make_blob(bufptr, save_len);
	put_env(q, term2, term2_ctx, tmp, -1);
	tmp->refcnt--;
	line = NULL;

	if (!session_readmsg(s, &line)) {
		q->is_yielded = 1;
		return 0;
	}

	// printf("-$$$ %s", line);
	free(line);
	return 1;
}

static int bif_http_put_chunk_2(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	stream *sp = term1->val_str;
	session *s = sp->sptr;

	session_set_cork(s, 1);
	char tmpbuf[256];
	snprintf(tmpbuf, sizeof(tmpbuf), "%x\r\n", (unsigned)term2->val_len);

	if (!session_writemsg(s, tmpbuf)) {
		session_set_cork(s, 0);
		return 0;
	}

	if (LEN_S(term2) > 0) {
		if (!session_write(s, VAL_S(term2), LEN_S(term2))) {
			session_set_cork(s, 0);
			return 0;
		}
	}

	snprintf(tmpbuf, sizeof(tmpbuf), "\r\n");

	if (!session_writemsg(s, tmpbuf)) {
		session_set_cork(s, 0);
		return 0;
	}

	session_set_cork(s, 0);
	return 1;
}

#if 0
static int bif_http_put_file_2(tpl_query *q)
{
	node *args = get_args(q);
	node *var; // FLAG_HIDDEN
	FILE *fp;

	if (!q->retry) {
		var = get_var(var);
	}
	else {
		var = get_stream(var);
	}

	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	stream *sp = term1->val_str, *sp2;
	session *s = sp->sptr;

	if (!q->retry) {
		fp = fopen(VAL_S(term2), "rb");

		if (!fp) {
			QABORT(ABORT_NOTEXISTFILE);
			return 0;
		}

		sp2 = calloc(1, sizeof(stream));
		sp2->fptr = fp;
		node *n = make_stream(sp2);
		put_env(q, var, var_ctx, n, -1);
		n->refcnt--;
		allocate_frame(q);
	}
	else {
		sp2 = var->val_str;
		fp = sp2->fptr;
	}

	if (!q->retry) {
		struct stat st;
		stat(VAL_S(term2), &st);
		char tmpbuf2[256];
		snprintf(tmpbuf2, sizeof(tmpbuf2), "%x\r\n", (unsigned)st.st_size);

		if (!session_writemsg(s, tmpbuf2)) {
			fclose(fp);
			sp2->fptr = NULL;
			return 0;
		}
	}

	size_t buflen = 1024 * 8;
	char *tmpbuf = malloc(buflen);
	size_t rlen;

	while ((rlen = fread(tmpbuf, 1, buflen, fp)) > 0) {
		const char *src = tmpbuf;

		while (rlen > 0) {
			int wlen = session_rawwrite(s, src, rlen);

			if (wlen < 0) {
				free(tmpbuf);
				fclose(fp);
				sp2->fptr = NULL;
				return 0;
			}

			if (!wlen) {
				free(tmpbuf);
				fseek(fp, -rlen, SEEK_CUR);
				process_yield_unlocked(q);
				return 0;
			}

			rlen -= wlen;
			src += wlen;
		}
	}

	free(tmpbuf);
	fclose(fp);
	sp2->fptr = NULL;
	char tmpbuf2[256];
	snprintf(tmpbuf2, sizeof(tmpbuf2), "\r\n0\r\n\r\n");

	if (!session_writemsg(s, tmpbuf2))
		return 0;

	return 1;
}
#endif

static int bif_ws_msg_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_atom(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	const char *op = VAL_S(term2);
	int bin = is_blob(term3);
	const char *src = VAL_S(term3);
	const uint64_t len = LEN_S(term3);

	if (!strcmp(op, "ping")) {
		if (!session_ws_ping(s, src, len))
			return 0;
	}
	else if (!strcmp(op, "close")) {
		if (!session_ws_close(s, src, len))
			return 0;
	}
	else if (!strcmp(op, "data")) {
		if (!session_ws_data(s, 1, bin, src, len))
			return 0;
	}
	else if (!strcmp(op, "more")) {
		if (!session_ws_data(s, 0, bin, src, len))
			return 0;
	}
	else
		return 0;

	return 1;
}

#if USE_SSL
static int ws_request(session *s, const char *path, const char *prots, int *status)
{
	char tmpbuf[256];
	snprintf(tmpbuf, sizeof(tmpbuf), "%lld-%d", (long long)time(NULL), rand_r(get_seed(s)));
	char key[256];
	key[0] = '\0';
	char *ptr = key;
	b64_encode(tmpbuf, strlen(tmpbuf), &ptr, 0, 0);

	const char *host = session_get_stash(s, "HOST");
	const char *user = session_get_stash(s, "USER");
	const char *pass = session_get_stash(s, "PASS");
	char dstbuf[1024 * 8];
	char *dst = dstbuf;
	dst += snprintf(dst, 1024 * 4, "GET %s HTTP/1.1\r\n", path);
	dst += snprintf(dst, 256, "Host: %s\r\n", host);

	if (user[0]) {
		dst += snprintf(dst, 256, "Authorization: Basic ");
		char tmpbuf[1024];
		snprintf(tmpbuf, sizeof(tmpbuf), "%s:%s", user, pass);
		dst += b64_encode(tmpbuf, strlen(tmpbuf), &dst, 0, 0);
		dst += sprintf(dst, "\r\n");
	}

	// dst += snprintf(dst, 256, "Origin: http://%s\r\n", host);
	dst += sprintf(dst, "Sec-WebSocket-Key: %s\r\n", key);

	if (prots)
		dst += snprintf(dst, 256, "Sec-WebSocket-Protocol: %s\r\n", prots);

	dst += sprintf(dst, "Sec-WebSocket-Version: %d\r\n", 13);
	dst += sprintf(dst, "Upgrade: websocket\r\n");
	dst += sprintf(dst, "Connection: Upgrade\r\n");
	sprintf(dst, "User-Agent: Trealla\r\n\r\n");
	session_writemsg(s, dstbuf);

	strcpy(dstbuf, key);
	strcat(dstbuf, WS_GUID);
	unsigned char digest[SHA_DIGEST_LENGTH];
	SHA1((unsigned char *)dstbuf, strlen(dstbuf), digest);
	ptr = key;
	b64_encode((const char *)digest, sizeof(digest), &ptr, 0, 0);

	char *bufptr = NULL;
	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		if (!session_get_udata_flag(s, CMD)) {
			session_set_udata_flag(s, CMD);
			char ver[20];
			ver[0] = '\0';
			sscanf(bufptr, "HTTP/%19s %d", ver, status);
			free(bufptr);
			ver[sizeof(ver) - 1] = '\0';
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);

			if (*status != 101)
				return 1;

			const char *tmpkey = session_get_stash(s, "Sec-WebSocket-Accept");

			if (strcmp(key, tmpkey)) {
				printf("*** WARN security missmatch, %s != %s\n", key, tmpkey);
				return 0;
			}

			session_set_websocket(s);
			return 1;
		}

		parse_header(s, bufptr, len);
	}

	return 0;
}

static int ws_upgrade(session *s, const char *prot)
{
	const char *sec_key = session_get_stash(s, "Sec-WebSocket-Key");
	char dstbuf[1024 * 8];
	char *dst = dstbuf;
	dst += snprintf(dst, 1024, "%s", sec_key);
	dst += sprintf(dst, "%s", WS_GUID);
	unsigned char digest[SHA_DIGEST_LENGTH];
	SHA1((unsigned char *)dstbuf, dst - dstbuf, digest);
	char key[256];
	char *ptr = key;
	b64_encode((const char *)digest, sizeof(digest), &ptr, 0, 0);

	dst = dstbuf;
	dst += sprintf(dst, "HTTP/1.1 101 Switching Protocols\r\n");
	dst += snprintf(dst, 256, "Sec-WebSocket-Protocol: %s\r\n", prot);
	dst += sprintf(dst, "Sec-WebSocket-Accept: %s\r\n", key);
	strcat(dst, "Upgrade: websocket\r\nConnection: Upgrade\r\nServer: "
	            "Trealla\r\n\r\n");
	session_writemsg(s, dstbuf);
	session_set_websocket(s);
	return 1;
}

static int bif_ws_request_5(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	node *term4 = get_atom_or_list(term4);
	node *term5 = get_var(term5);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	const char *path = VAL_S(term2);

	char prots[256];
	prots[0] = '\0';

	if (is_atom(term4) && strcmp(VAL_S(term4), "[]"))
		strcat(prots, VAL_S(term4));

	node *l = term4;

	while (is_list(l)) {
		node *head = term_firstarg(l);
		node *n = subst(q, head, q->c.curr_frame);

		if (is_atom(n)) {
			if (prots[0])
				strcat(prots, ",");

			strcat(prots, VAL_S(n));
		}

		node *tail = term_next(head);
		l = subst(q, tail, q->latest_context);
	}

	int status;

	int ok = ws_request(s, path, prots, &status);
	put_int(q, term3, term3_ctx, status);

	if (ok)
		put_atom(q, term5, term5_ctx, strdup(session_get_stash(s, "Sec-WebSocket-Protocol")));

	return 1;
}

static int bif_ws_request_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	const char *path = VAL_S(term2);
	int status;

	ws_request(s, path, NULL, &status);
	put_int(q, term3, term3_ctx, status);
	return 1;
}

static int bif_ws_upgrade_2(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	const char *prot = VAL_S(term2);

	return ws_upgrade(s, prot);
}
#endif

static int bif_ws_parse_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_term(term1);
	node *term2 = get_var(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;

	if (!is_socket(term1)) {
		q->halt_code = 1;
		q->halt = ABORT_HALT;
		return 0;
	}

	if (session_on_disconnect(s)) {
		q->is_yielded = 1;
		return 0;
	}

	int fin;
	unsigned opcode = 0;
	size_t dstlen = 0;
	char *dstbuf = NULL;

	if (!session_ws_parse(s, &fin, &opcode, &dstbuf, &dstlen)) {
		q->is_yielded = 1;
		return 0;
	}

	node *n = NULL;

	if (opcode == WS_OP_BINARY)
		n = make_blob(dstbuf, dstlen);
	else
		n = make_atom(dstbuf);

	const char *op = "";

	if (opcode == WS_OP_CLOSE)
		op = "close";
	else if (opcode == WS_OP_PONG)
		op = "pong";
	else if (opcode == WS_OP_PING)
		op = "ping";
	else if (fin)
		op = "data";
	else
		op = "more";

	put_const_atom(q, term2, term2_ctx, op);
	put_env(q, term3, term3_ctx, n, -1);
	n->refcnt--;
	return 1;
}

static int bif_ws_is_ws(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_term(term2);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	int ok = session_is_websocket(s);
	node *n = make_const_atom(ok ? "true" : "false");
	n->refcnt++;
	ok = unify(q, term2, term2_ctx, n, q->c.curr_frame);
	term_heapcheck(n);
	return ok;
}

static int bif_h2_request_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	const char *host = session_get_stash(s, "HOST");
	const char *user = session_get_stash(s, "USER");
	const char *pass = session_get_stash(s, "PASS");
	size_t mlen = strlen(VAL_S(term2)) + strlen(host) + 1024;
	char *dstbuf = (char *)malloc(mlen);
	char *dst = dstbuf;
	dst += sprintf(dst, "GET %s HTTP/1.1\r\n", VAL_S(term2));
	dst += sprintf(dst, "Host: %s\r\n", host);

	if (user[0]) {
		dst += snprintf(dst, 256, "Authorization: Basic ");
		char tmpbuf[1024];
		snprintf(tmpbuf, sizeof(tmpbuf), "%s:%s", user, pass);
		dst += b64_encode(tmpbuf, strlen(tmpbuf), &dst, 0, 0);
		dst += sprintf(dst, "\r\n");
	}

	dst += sprintf(dst, "HTTP2-Settings: %s\r\n", "???");
	dst += sprintf(dst, "Upgrade: h2c\r\n");
	dst += sprintf(dst, "Connection: Upgrade,HTTP2-Settings\r\n");
	sprintf(dst, "User-Agent: Trealla\r\n\r\n");
	session_writemsg(s, dstbuf);

	char *bufptr = NULL;
	int status = 0;
	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		if (!session_get_udata_flag(s, CMD)) {
			session_set_udata_flag(s, CMD);
			char ver[20];
			ver[0] = '\0';
			sscanf(bufptr, "HTTP/%19s %d", ver, &status);
			free(bufptr);
			ver[sizeof(ver) - 1] = '\0';
			put_int(q, term3, term3_ctx, status);
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);
			if (status != 101)
				return 1;
			session_set_udata_flag(s, HTTP2);
			return 1;
		}

		parse_header(s, bufptr, len);
	}

	return 0;
}

static int bif_h2_upgrade_2(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	// const char *sec_set = session_get_stash((session*)sp->sptr,
	// "HTTP2-Settings");
	char dstbuf[1024 * 4];
	char *dst = dstbuf;
	dst += sprintf(dst, "HTTP/1.1 101 Switching Protocols\r\n");
	// dst += snprintf(dst, 1024, "%s\r\n", sec_set);
	strcat(dst, "Upgrade: h2c\r\nConnection: Upgrade\r\nServer: Trealla\r\n\r\n");
	session_writemsg(s, dstbuf);
	session_set_udata_flag(s, HTTP2);
	return 1;
}

static int bif_h2_msg_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_int(term2);
	node *term3 = get_atom(term3);
	// stream *sp = term1->val_str;
	// session *s = sp->sptr;
	// const uint64_t len = LEN_S(term3);
	return 0;
}

static int bif_h2_parse_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_term(term1);
	node *term2 = get_var(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;

	if (!is_socket(term1)) {
		q->halt_code = 1;
		q->halt = ABORT_HALT;
		return 0;
	}

	if (session_on_disconnect(s))
		return 0;

	return 0;
}

static int bif_h2_is_h2_2(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_term(term2);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	int ok = session_get_udata_flag(s, HTTP2);
	node *n = make_const_atom(ok ? "true" : "false");
	n->refcnt++;
	ok = unify(q, term2, term2_ctx, n, q->c.curr_frame);
	term_heapcheck(n);
	return ok;
}

#if 0
static char *stomp_escape(const char *path, char *path2)
{
	const char *src = path;
	char *dst = path2;

	while (*src)
	{
		if (*src == ':')
		{
			*dst++ = '\\';
			*dst++ = 'c';
		}
		else if (*src == '\\')
		{
			*dst++ = '\\';
			*dst++ = '\\';
		}
		else
			*dst++ = *src;

		src++;
	}

	*dst = '\0';
	return path2;
}
#endif

static char *stomp_deescape(const char *path, char *path2)
{
	const char *src = path;
	char *dst = path2;

	while (isspace(*src))
		src++;

	while (*src) {
		if (*src == '\\') {
			src++;

			if (*src == 'r')
				*dst++ = '\r';
			else if (*src == 'n')
				*dst++ = '\n';
			else if (*src == 'c')
				*dst++ = ':';
			else if (*src == '\\')
				*dst++ = '\\';
		}
		else {
			if ((src[0] == '.') && (src[1] == '.'))
				src += 2;
			else
				*dst++ = *src;
		}

		src++;
	}

	*dst = '\0';
	return path2;
}

static int bif_stomp_parse_3(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_term(term1);
	node *term2 = get_var(term2);
	node *term3 = get_var(term3);
	stream *sp = term1->val_str;
	session *s = sp->sptr;
	char *bufptr = NULL;

	if (!is_socket(term1)) {
		q->halt_code = 1;
		q->halt = ABORT_HALT;
		return 0;
	}

	if (session_on_disconnect(s)) {
		q->is_yielded = 1;
		return 0;
	}

	int len;

	while ((len = session_readmsg(s, &bufptr)) > 0) {
		if (!session_get_udata_flag(s, CMD)) {
			session_clr_stash(s);
			session_set_udata_flag(s, CMD);

			char cmd[256];
			cmd[0] = '\0';
			sscanf(bufptr, "%255s", cmd);
			free(bufptr);
			cmd[sizeof(cmd) - 1] = '\0';
			put_atom(q, term3, term3_ctx, strdup(cmd));
			session_set_stash(s, "REQUEST_METHOD", cmd);

			if (session_is_ipv6(s)) {
				char tmpbuf[256];
				snprintf(tmpbuf, sizeof(tmpbuf), "[%s]", session_get_remote_addr(s, 0));
				session_set_stash(s, "REMOTE_ADDR", tmpbuf);
				session_set_stash(s, "REMOTE_HOST", tmpbuf);
				snprintf(tmpbuf, sizeof(tmpbuf), "[%s]", session_get_local_addr(s, 0));
				session_set_stash(s, "LOCAL_ADDR", tmpbuf);
				session_set_stash(s, "LOCAL_HOST", tmpbuf);
			}
			else {
				session_set_stash(s, "REMOTE_ADDR", session_get_remote_addr(s, 0));
				session_set_stash(s, "REMOTE_HOST", session_get_remote_addr(s, 0));
				session_set_stash(s, "LOCAL_ADDR", session_get_local_addr(s, 0));
				session_set_stash(s, "LOCAL_HOST", session_get_local_addr(s, 0));
			}

			session_set_stash_int(s, "REMOTE_PORT", session_get_remote_port(s));
			session_set_stash_int(s, "LOCAL_PORT", session_get_local_port(s));
			continue;
		}

		if ((bufptr[0] == '\r') || (bufptr[0] == '\n')) {
			free(bufptr);
			session_clr_udata_flag(s, CMD);
			char tmpbuf[1024], tmpbuf2[1024], tmpbuf3[256];
			tmpbuf2[0] = tmpbuf3[0] = '\0';
			const char *src = session_get_stash(s, "Host");
			sscanf(src, "%1023[^: \r\n]:%255s", tmpbuf2, tmpbuf3);
			tmpbuf2[sizeof(tmpbuf2) - 1] = tmpbuf3[sizeof(tmpbuf3) - 1] = '\0';

			if (tmpbuf2[0])
				session_set_stash(s, "SERVER_NAME", stomp_deescape(tmpbuf2, tmpbuf));
			else
				session_set_stash(s, "SERVER_NAME", session_get_stash(s, "LOCAL_ADDRESS"));

			if (tmpbuf3[0])
				session_set_stash(s, "SERVER_PORT", stomp_deescape(tmpbuf3, tmpbuf));
			else
				session_set_stash(s, "SERVER_PORT", session_get_stash(s, "LOCAL_PORT"));

			if (strlen(session_get_stash(s, "Authorization"))) {
				const char *hdr = session_get_stash(s, "Authorization");
				char auth[256], tmpbuf[256];
				auth[0] = tmpbuf[0] = '\0';
				sscanf(hdr, "%255s %255s", auth, tmpbuf);
				auth[sizeof(auth) - 1] = tmpbuf[sizeof(tmpbuf) - 1] = '\0';

				if (!strcmp(auth, "Basic")) {
					size_t nbytes = 64;
					char *dstbuf = (char *)malloc(nbytes);
					b64_decode(tmpbuf, strlen(tmpbuf), (char **)&dstbuf);
					char userid[256], passwd[256];
					userid[0] = passwd[0] = '\0';
					sscanf(dstbuf, "%255[^:]:%255s", userid, passwd);
					userid[sizeof(userid) - 1] = passwd[sizeof(passwd) - 1] = '\0';
					session_set_stash(s, "USER", userid);
					session_set_stash(s, "PASS", passwd);
					free(dstbuf);
				}
			}

			if (strlen(session_get_stash(s, "WWW-Authenticate"))) {
				const char *hdr = session_get_stash(s, "WWW-Authenticate");
				char auth[256], tmpbuf[256];
				auth[0] = tmpbuf[0] = '\0';
				sscanf(hdr, "%255s %255s", auth, tmpbuf);
				auth[sizeof(auth) - 1] = tmpbuf[sizeof(tmpbuf) - 1] = '\0';

				if (!strcmp(auth, "Basic") && !strncmp(tmpbuf, "Realm", 5)) {
					const char *src = tmpbuf;
					char *dst = auth;

					while (isalpha(*src) || (*src == '='))
						src++;

					if (*src == '"')
						src++;

					while (*src && (*src != '"') && ((dst - auth) < (sizeof(auth) - 1)))
						*dst++ = *src++;

					*dst = '\0';
					session_set_stash(s, "REALM", auth);
				}
			}

			if (strlen(session_get_stash(s, "Content-Length")))
				session_set_stash(s, "CONTENT_LENGTH",
				                  session_get_stash(s, "Content-Length"));
			if (strlen(session_get_stash(s, "Content-Type")))
				session_set_stash(s, "CONTENT_TYPE", session_get_stash(s, "Content-Type"));

			if (strlen(session_get_stash(s, "Version"))) {
				put_float(q, term2, term2_ctx, atof(session_get_stash(s, "Version")));
				session_set_stash(s, "STOMP", session_get_stash(s, "Version"));
			}
			else if (strlen(session_get_stash(s, "Accept-Version"))) {
				const char *verstr = session_get_stash(s, "Accept-Version");
				double ver;

				if (strstr(verstr, "1.2"))
					put_float(q, term2, term2_ctx, ver = 1.2);
				else if (strstr(verstr, "1.1"))
					put_float(q, term2, term2_ctx, ver = 1.1);
				else
					put_float(q, term2, term2_ctx, ver = 1.0);

				char tmpbuf[256];
				snprintf(tmpbuf, sizeof(tmpbuf), "%.1f", ver);
				session_set_stash(s, "STOMP", tmpbuf);
			}

			return 1;
		}

		parse_header(s, bufptr, len);
	}

	q->is_yielded = 1;
	return 0;
}

static int bif_stomp_msg_4(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_socket(term1);
	node *term2 = get_atom(term2);
	node *term3 = get_atom(term3);
	node *term4 = get_atom(term4);
	stream *sp = term1->val_str;
	session *s = sp->sptr;

	session_set_cork(s, 1);
	session_writemsg(s, VAL_S(term2));
	session_writemsg(s, "\n");
	size_t len = LEN_S(term3);

	if (len) {
		const char *src = VAL_S(term3);
		char *tmpbuf = (char *)malloc((len * 2) + 1);
		char *dst = tmpbuf;

		while (*src) {
			while (*src != ':')
				*dst++ = *src++;

			*dst++ = *src++; // the ':'

			while ((*src != 'r') && (*src != '\n')) {
				if (*src == ':') {
					*dst++ = '\\';
					*dst++ = 'c';
				}
				else if (*src == '\\') {
					*dst++ = '\\';
					*dst++ = '\\';
				}
				else
					*dst++ = *src;

				src++;
			}

			while ((*src == 'r') || (*src == '\n'))
				*dst++ = *src++;
		}

		*dst = '\0';
		session_writemsg(s, tmpbuf);
		free(tmpbuf);
	}

	len = LEN_S(term4);
	char tmpbuf[256];
	snprintf(tmpbuf, sizeof(tmpbuf), "content-length:%u\n\n", (unsigned)len);
	session_writemsg(s, tmpbuf);
	session_write(s, VAL_S(term4), len + 1); // Include the NULL
	session_set_cork(s, 0);
	return 1;
}

void bifs_load_http(void)
{
	DEFINE_BIF("http:parse", 4, bif_http_parse_4);
	DEFINE_BIF("http:parse", 3, bif_http_parse_3);
	DEFINE_BIF("http:parse", 2, bif_http_parse_3);
	DEFINE_BIF("http:www_form", 1, bif_http_www_form_1);

	DEFINE_BIF("http:query", 3, bif_http_query_3);
	DEFINE_BIF("http:form", 3, bif_http_form_3);
	DEFINE_BIF("http:cookie", 3, bif_http_cookie_3);

	DEFINE_BIF("http:query_list", 3, bif_http_query_list_3);
	DEFINE_BIF("http:form_list", 3, bif_http_form_list_3);
	DEFINE_BIF("http:cookie_list", 3, bif_http_cookie_list_3);

	DEFINE_BIF("http:head", 2, bif_http_head_3);
	DEFINE_BIF("http:head", 3, bif_http_head_3);
	DEFINE_BIF("http:head", 4, bif_http_head_3);
	DEFINE_BIF("http:get", 2, bif_http_get_3);
	DEFINE_BIF("http:get", 3, bif_http_get_3);
	DEFINE_BIF("http:get", 4, bif_http_get_3);
	DEFINE_BIF("http:post", 2, bif_http_post_3);
	DEFINE_BIF("http:post", 3, bif_http_post_3);
	DEFINE_BIF("http:post", 4, bif_http_post_3);
	DEFINE_BIF("http:put", 2, bif_http_put_3);
	DEFINE_BIF("http:put", 3, bif_http_put_3);
	DEFINE_BIF("http:put", 4, bif_http_put_3);
	DEFINE_BIF("http:delete", 2, bif_http_delete_3);
	DEFINE_BIF("http:delete", 3, bif_http_delete_3);
	DEFINE_BIF("http:delete", 4, bif_http_delete_3);

	DEFINE_BIF("http:get_chunk", 3, bif_http_get_chunk_3);
	DEFINE_BIF("http:put_chunk", 2, bif_http_put_chunk_2);

	DEFINE_BIF("h2:is_h2", 2, bif_h2_is_h2_2);
	DEFINE_BIF("h2:request", 3, bif_h2_request_3);
	DEFINE_BIF("h2:upgrade", 2, bif_h2_upgrade_2);
	DEFINE_BIF("h2:parse", 3, bif_h2_parse_3);
	DEFINE_BIF("h2:msg", 3, bif_h2_msg_3);
}

void bifs_load_ws(void)
{
#if USE_SSL
	DEFINE_BIF("ws:request", 5, bif_ws_request_5);
	DEFINE_BIF("ws:request", 3, bif_ws_request_3);
	DEFINE_BIF("ws:upgrade", 2, bif_ws_upgrade_2);
#endif

	DEFINE_BIF("ws:parse", 3, bif_ws_parse_3);
	DEFINE_BIF("ws:msg", 3, bif_ws_msg_3);
	DEFINE_BIF("ws:is_ws", 2, bif_ws_is_ws);
}

void bifs_load_stomp(void)
{
	DEFINE_BIF("stomp:parse", 3, bif_stomp_parse_3);
	DEFINE_BIF("stomp:msg", 4, bif_stomp_msg_4);
}
