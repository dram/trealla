...
#if !defined(BSD)
#if (defined(__bsdi__) || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__NetBSD__) || defined(__Darwin__) || defined (__DragonFly__))
#define BSD 1
#endif
#endif
...
int session_enable_tls(session *s, const char *certfile, int level)
{
	...
	if (s->is_client)
	{
		if (!s_ctx)
		{
			...
			SSL_CTX_set_cipher_list(s_ctx, "HIGH:!aNULL");
	...
	if (!s->is_client)
	{
		...
		{
			if (g_debug) printf("SSL_accept failed\n");
	...
	else
	{
		...
		{
			if (g_debug) printf("SSL_connect failed\n");
			if (g_debug) ERR_print_errors_fp(stderr);
			return 0;
		}
	}
...
void session_lock(session *s) {	if (s->h) lock_lock(s->h->strand); }
void session_unlock(session *s) { if (s->h) lock_unlock(s->h->strand); }
...
static const char *inet_ntop(int family, void *address, char *buffer, socklen_t len)
{
	...
	if (family == AF_INET6)
	{
		...
		if (WSAAddressToString((struct sockaddr*)&sin6,
			sizeof(sin6), NULL, buffer, &buflen) == SOCKET_ERROR)
				strcpy(buffer, "");
	}
	else
	{
		...
		if (WSAAddressToString((struct sockaddr*)&sin4,
			sizeof(sin4), NULL, buffer, &buflen) == SOCKET_ERROR)
				strcpy(buffer, "");
	}
...
int ws_msg(session *s, unsigned fin, unsigned opcode, const char *src, size_t len)
{
	...
	if (masked)
	{
		union { uint32_t mask; char mbytes[4]; } m;
		m.mask = rand();
...
static int session_shutdown(session *s)
{
	if (s->fd != -1)
...
{
	if (g_debug) printf("*** KQUEUE: accept %d\n", ((session*)data)->fd);
...
{
	if (g_debug) printf("*** KQUEUE: close %d\n", ((session*)data)->fd);
	session *s = (session*)data;
	struct kevent ev = {0};
	...
	while (!g_abort && !h->halt && (h->use || wait))
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (s->disconnected)
			{
				...
				tpool_start(h->tp, &kqueue_close, s);
				h->use--;
			}
			else
				tpool_start(h->tp, &kqueue_run, s);
...
{
	if (g_debug) printf("*** EPOLL: accept %d\n", ((session*)data)->fd);
	session *s = (session*)data;
	s->f(s, s->v);
...
{
	if (g_debug) printf("*** EPOLL: close %d\n", ((session*)data)->fd);
	session *s = (session*)data;
	struct epoll_event ev = {0};
...
int handler_wait_epoll(handler *h, int wait)
{
	...
	while (!g_abort && !h->halt && (h->use || wait))
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if ((int)events[i].data.u64 < h->cnt)
			{
			...
			{
				sb_int_del(h->fds, s->fd);
				tpool_start(h->tp, &epoll_close, s);
				h->use--;
			}
			else
				tpool_start(h->tp, &epoll_run, s);
...
{
	if (g_debug) printf("*** POLL: accept %d\n", ((session*)data)->fd);
	session *s = (session*)data;
	s->f(s, s->v);
...
{
	if (g_debug) printf("*** POLL: close %d\n", ((session*)data)->fd);
	session *s = (session*)data;
	s->f(s, s->v);
	session_close(s);
	session_unshare(s);
...
{
	if (g_debug) printf("*** SELECT: accept %d %d\n", ((session*)data)->fd, ((session*)data)->fd);
	session *s = (session*)data;
	s->f(s, s->v);
	s->h->use++;
	return 1;
...
{
	if (g_debug) printf("*** SELECT: close %d\n", ((session*)data)->fd);
	session *s = (session*)data;
	s->f(s, s->v);
	session_close(s);
	session_unshare(s);
...
static int handler_select_bads(void *_h, int fd, void *_s)
{
	...
	session *s = (session*)_s;
	sb_int_del(h->fds, fd);
	tpool_start(s->h->tp, &select_close, s);
	h->use--;
	return 1;
}
...
int handler_wait_indefinitely(handler *h)
{
	return handler_wait2(h, 1);
}

int handler_wait(handler *h)
{
	return handler_wait2(h, 0);
}
...
int handler_set_tls(handler *h, const char *keyfile, const char *certfile)
{
	...
	SSL_CTX_set_cipher_list(h->ctx, "HIGH:!aNULL");
...
static int handler_add_server2(handler *h, int (*f)(session*, void *v), void *v,
	const char *binding, unsigned port6, unsigned port4, int tcp, int ssl, int pri,
...
int handler_add_server(handler *h, int (*f)(session*, void *v), void *v,
	const char *binding, unsigned port6, unsigned port4, int tcp, int ssl, int pri, const char *name)
{
...
int handler_add_multicast(handler *h, int (*f)(session*, void *v), void *v,
	const char *binding, unsigned port, const char *addr6, const char *addr4, const char *name)
{
