...
#define SIZEOF_SMALL_ATOM sizeof(list)
...
struct node_ {
	...
	union {
		...
		char val_ch[SIZEOF_SMALL_ATOM]; // small atoms
...
struct lexer_ {
	...
	int fact, dcg, dcg_passthru, consult, quoted, was_atomic, save_vars, vars;
	int internal, comment, was_op, is_op, was_op2, is_op2, is_spaced, was_spaced;
	int flag_unknown, flag_character_escapes, flag_char_conversion, expandable;
...
typedef struct {
	...
	uint16_t frame_size, trail_size;
...
enum {
	...
	ABORT_INVALIDARGNOTCOMPOUNDORVAR,
	ABORT_INVALIDARGNOTPREDICATEINDICATOR,
