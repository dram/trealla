...
int check_builtin(trealla *pl, const char *functarity)
{
...
int check_dynamic(module *db, const char *functarity)
{
...
int check_static(module *db, const char *functarity)
{
	char *key = (char *)functarity;
	rule *r = NULL;
...
int read_term(tpl_query *q, char *line, node *term1, unsigned term1_ctx, node *term2, FILE *fp)
{
	if (!*line)
		return 0;
...
int bif_iso_get_code(tpl_query *q)
{
	node *args = get_args(q);
	node *term1 = get_int_or_var(term1);
...
void rebase_clause(tpl_query *q, node *n)
{
	skiplist vars;
	sl_init(&vars, NULL, NULL);
	q->d = &vars;
...
int bif_retract2(tpl_query *q, int wait)
{
	node *args = get_args(q);
	node *term1 = get_callable(term1);
	const char *functor;
...
int bif_iso_clause(tpl_query *q) { return bif_clause(q, 0); }
...
void eval(tpl_query *q, node **args)
{
