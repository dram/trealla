...
static int bif_iso_get_char(tpl_query *q)
{
	...
	if (ch == EOF) {
		int ok = unify_const_atom(q, term1, END_OF_FILE, 0);
		q->did_getc = 0;
		return ok;
	}
	...
	return unify_atom(q, term1, strdup(tmpbuf), needs_quoting(tmpbuf));
}
...
static int bif_iso_get_char_2(tpl_query *q)
{
	...
	char tmpbuf[20];
	put_char_utf8(tmpbuf, ch);
	return unify_atom(q, term2, strdup(tmpbuf), needs_quoting(tmpbuf));
}
...
static int bif_iso_peek_char(tpl_query *q)
{
	...
	if (ch == EOF)
		return unify_const_atom(q, term1, END_OF_FILE, 0);
...
static int bif_iso_number_chars(tpl_query *q)
{
	...
	while (*src) {
		node *tmp = make_atom(strndup(src, 1), 0);
		term_append(l, tmp);
...
static int bif_iso_atom_chars(tpl_query *q)
{
	...
	if (is_list(term2)) {
		...
		*dst = '\0';
		int ok = unify_atom(q, term1, strdup(dstbuf), needs_quoting(dstbuf));
		free(dstbuf);
		return ok;
	}
	...
	while (*src) {
		...
		char tmpbuf[20];
		put_char_utf8(tmpbuf, ch);
		node *tmp = make_atom(strdup(tmpbuf), needs_quoting(tmpbuf));
		term_append(l, tmp);
...
static int bif_iso_char_code(tpl_query *q)
{
	...
	if (is_integer(term2)) {
		...
		tmpbuf[1] = '\0';
		int ok = unify_atom(q, term1, strdup(tmpbuf), needs_quoting(tmpbuf));
		return ok;
	}
...
static int bif_xtra_term_to_atom_2(tpl_query *q)
{
	...
	if (!is_atom(term1)) {
		...
		ok = unify_atom(q, term2, strdup(tmpbuf), needs_quoting(tmpbuf));
		free(tmpbuf);
	}
	else
...
static int bif_xtra_seeing_1(tpl_query *q)
{
	...
	if (q->curr_stdin_stream) {
		...
		return unify_atom(q, term1, strdup(q->curr_stdin_name), needs_quoting(q->curr_stdin_name));
}
...
static int bif_xtra_telling_1(tpl_query *q)
{
	...
	if (q->curr_stdout_stream) {
		...
		return unify_atom(q, term1, strdup(q->curr_stdout_name), needs_quoting(q->curr_stdout_name));
}
...
static int bif_xtra_getenv_2(tpl_query *q)
{
	...
	return unify_atom(q, term2, strdup(value), needs_quoting(value));
}
	...
	if (is_list(term2)) {
		...
		*dst = '\0';
		...
		if (isdigit(*dstbuf) && (numeric == NUM_INT))
			...
			ok = unify_atom(q, term1, strdup(dstbuf), needs_quoting(dstbuf));
